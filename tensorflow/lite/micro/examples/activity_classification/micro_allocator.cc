...
//  lcd.DisplayStringAt(0, LINE(6), (uint8_t *)"init SB", CENTER_MODE);
  TF_LITE_ENSURE_STATUS(InitScratchBufferData());

//  lcd.DisplayStringAt(0, LINE(7), (uint8_t *)"AllocateTfLiteEvalTensors", CENTER_MODE);
  TF_LITE_ENSURE_STATUS(AllocateTfLiteEvalTensors(model, eval_tensors));

//  lcd.DisplayStringAt(0, LINE(8), (uint8_t *)"AllocateNodeAndRegistrations", CENTER_MODE);
  TF_LITE_ENSURE_STATUS(
      AllocateNodeAndRegistrations(model, node_and_registrations));

//  lcd.DisplayStringAt(0, LINE(9), (uint8_t *)"PrepareNodeAndRegistrationDataFromFlatbuffer", CENTER_MODE);
